...
const rowEdit = function (e) {
  ...
  if (target.classList.contains("td__edit--delete")) {
    ...
    fetch("php/jo_edit-inc.php", {
      method: "POST", // or 'PUT'
      headers: {
        "Content-Type": "application/x-www-form-urlencoded",
      },
      body: `delete&productId=${productId.innerHTML}&joId=${inputId.value}`,
...
const selectItem = function (e) {
  ...
  const joQty = prompt("Enter Qty-in");
  const itemDiscPercent = 0;
  const itemDiscVal = 0;
  const totalCost = joQty * itemPrice;
  ...
  tableItemTb.querySelector("tbody").insertAdjacentHTML(
   ...
   <i class="fa fa-trash-o" style="font-size:24px"></i>
    ...
    <input type='hidden' name='qtyIn[]' value='${joQty}' class='input__edit input__edit--qty'>
